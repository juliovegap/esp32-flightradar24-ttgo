...
// When an airport is selected in the config dialog, the latitude and longitude are set to the
// position of that airport without a round trip to the device: the position of the airport is
// requested with XHR and the fields are updated. Without this script the position is only applied
// when the configuration is saved (see AirportSelectParameter::applySelection).
const char CONFIG_PAGE_SCRIPT[] PROGMEM = R"js(
function airport_selected(select, latitude_id, longitude_id) {
  if (select.value.length == 0) return;
  var request = new XMLHttpRequest();
  request.onreadystatechange = function () {
    if (request.readyState != 4 || request.status != 200) return;
    var position = request.responseText.split(',');
    if (position.length != 2) return;
    var latitude = document.getElementById(latitude_id);
    var longitude = document.getElementById(longitude_id);
    // The position of an airport has more decimals than the step size of the fields
    latitude.step = 'any';
    longitude.step = 'any';
    latitude.value = position[0];
    longitude.value = position[1];
  };
  request.open('GET', '/airport?code=' + select.value, true);
  request.send();
}
)js";

// Adds the script above to the pages generated by IotWebConf
class ConfigPageFormatProvider : public iotwebconf::HtmlFormatProvider
{
protected:
  String getScriptInner() override
  {
    return iotwebconf::HtmlFormatProvider::getScriptInner() + FPSTR(CONFIG_PAGE_SCRIPT);
  }
};
auto config_page_format_provider = ConfigPageFormatProvider();

auto param_group = iotwebconf::ParameterGroup("flightradar", "Flight radar");
auto iotWebParamLocation = iotwebconf::Builder<AirportSelectParameter>("location")
...
// when it is not an airport. Empty means the latitude and longitude are used.
String get_location_name()
{
  const auto airport = lookup_airport(iotWebParamLocation.value());
  if (airport != nullptr)
    return airport->name;

  const auto location = String(iotWebParamLocation.value());
  return location.length() > 0 ? location : format_gps_location(iotWebParamLatitude.value(), iotWebParamLongitude.value());
}

// Configurations from an older version contain the name of the airport instead of the IATA
// code; replace it with the code so it can be selected in the drop down. The latitude and
// longitude are left untouched.
void migrate_location()
{
  const auto location = String(iotWebParamLocation.value());
  if (location.length() == 0 || lookup_airport(location.c_str()) != nullptr)
    return;

  for (size_t i = 0; i < airport_count; i++)
  {
    if (location.equalsIgnoreCase(airports[i].name))
    {
      ...
      log_i("Location %s is airport %s", location.c_str(), iata_airport);
      strncpy(iotWebParamLocation.value(), iata_airport, sizeof(iotWebParamLocation.value()));
      return;
    }
  }
}
...
void setup()
{
  ...
  // Position of an airport, used by the config dialog (see CONFIG_PAGE_SCRIPT)
  server.on("/airport", HTTP_GET, []()
            {
              const auto airport = lookup_airport(server.arg("code").c_str());
              if (airport == nullptr)
              {
              ...
              char position[32];
              snprintf(position, sizeof(position), "%.6f,%.6f", airport->latitude / 1000000.0, airport->longitude / 1000000.0);
...
void display_clock()
{
  ...
  if (time_valid())
  {
    ...
    lv_label_update_text(clock_location_label, format_to_latin(get_location_name().c_str()).c_str());
  }
  else
  {
  ...
  if (static_cast<long>(now - next_update) > 0)
  {
    ...
    if (flights.empty() || display_cycle >= display_cycles)
    {
      ...
      if (flights.empty())
      {
        ...
        lv_label_update_text(no_flights_location_label, format_to_latin(get_location_name().c_str()).c_str());
